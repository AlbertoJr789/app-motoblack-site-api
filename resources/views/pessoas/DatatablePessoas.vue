<template>
    ...
    <DataTable :columns="columns" :ajax="ajax" :options="options" ref="table" class="display responsive border border-transparent border-separate border-spacing-0 rounded-lg" id="tablePessoas">
...
<script setup>
    ...
    const options = {
        ...
        initComplete: () => {
            if(props.canDelete)
                handleCheckboxes(document.getElementById('tablePessoas'),checked)
        },
        drawCallback: () => {
            if(props.canDelete){
