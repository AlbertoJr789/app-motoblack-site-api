<template>
    ...
    <DataTable :columns="columns" :ajax="ajax" :options="options" ref="table" class="display responsive border border-transparent border-separate border-spacing-0 rounded-lg" id="tableVeiculos">
...
<script setup>
    ...
    const columns = [
        ...
        {data: 'id', title: window.trans('id')},{data: 'tipo', title: window.trans('tipo')},{data: 'modelo', title: window.trans('modelo')},{data: 'marca', title: window.trans('marca')},{data: 'chassi', title: window.trans('chassi')},{data: 'renavam', title: window.trans('renavam')},{data: 'placa', title: window.trans('placa')},{data: 'cor', title: window.trans('cor')},{data: 'data_desativacao', title: window.trans('data_desativacao')},{data: 'agente_id', title: window.trans('agente_id')},{data: 'created_at', title: window.trans('Creation Date')},{data: 'updated_at', title: window.trans('Update Date')},
    ...
    const options = {
        ...
        initComplete: () => {
            if(props.canDelete)
                handleCheckboxes(document.getElementById('tableVeiculos'),checked)
        },
        drawCallback: () => {
            if(props.canDelete){
