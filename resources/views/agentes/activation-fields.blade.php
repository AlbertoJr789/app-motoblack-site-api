...
<h1 class="text-2xl mt-4">{{__('Address')}}</h1>
<span>{{__('Insert the vehicle data just the way it is presented in the document')}}.{{__('Make the necessary changes or deactivate the agent if the fields don\'t match to the document that was sent')}}</span>
<livewire:address-fields :required="true" key={{now()}} :addr="$Agente?->pessoa?->endereco"/>

<h1 class="text-2xl mt-4">{{__('Vehicle Info')}}</h1>
